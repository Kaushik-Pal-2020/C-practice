...
void sumRootToLeafutil(BinaryTree* root, int flag, int *power);
int sumRootToLeaf(BinaryTree* root);
...
void sumRootToLeafutil(BinaryTree* root, int flag, int *total)
{
    static int data = 0;
    if (flag == 1)
    {
    ...
    if (root)
    {
        ...
        sumRootToLeafutil(root->lchild, 0, total);
        sumRootToLeafutil(root->rchild, 0, total);
        data >>=1;
    }
}
