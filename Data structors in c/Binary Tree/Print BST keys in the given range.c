...
void printInRange(BST_node *root, int startValue, int endValue)
{
    static unsigned short flag = 1;
    while (root && flag)
    {
        printInRange(root->lchild, startValue, endValue);

        if (root->data >= startValue && root->data <= endValue)
            printf("%d ", root->data);
        else if (root->data > endValue)
            flag = 1;

        printInRange(root->rchild, startValue, endValue);
        return;
    }
}
