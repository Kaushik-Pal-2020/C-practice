...
void isUnivalTreeutil(BinaryTree* root, int *data, int *flag);
...
void isUnivalTreeutil(BinaryTree* root, int *data, int* flag)
{
    ...
    if (root && (*flag != 0))
    {
        if (root->data != *data)
            *flag = 0;
        isUnivalTreeutil(root->lchild, data, flag);
        isUnivalTreeutil(root->rchild, data, flag);
    }
}
bool isUnivalTree(BinaryTree* root)
{
    ...
    isUnivalTreeutil(root, &data, &flag);
